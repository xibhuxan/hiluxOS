...
Última actualización: 2026-09-05 (Media: **carpetas configurables persistidas en BD** — tabla `media_folders` + CRUD API + raíl con añadir/quitar y aviso de ruta no disponible; **modo pantalla completa táctil** compartido para Media/Radio; **fix seed one-shot**: `MEDIA_DIR` ya no se re-siembra tras borrar las carpetas; **pulido UI Fase 4**: shimmer en búsqueda de Radio + strip now-playing con badge EN DIRECTO + pop de favorito + highlight de emisora actual, `SectionHeader` compartido en System/Settings, celda Versión + hint de actualización en la card Sistema del Home; roadmap en `docs/ROADMAP-MEDIA.md`)
